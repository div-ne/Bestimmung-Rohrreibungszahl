...
def calculate_lambda(fluid, temperature_c, pressure_bar, diameter_mm, velocity_ms, roughness_mm):
    ...
    if re < 2320:
        ...
        basis_text = "Übergangsbereich zwischen hydraulisch glatt und hydraulisch rau"
    overview = pd.DataFrame(
        [
            ...
            ("Rohrrauhigkeit [mm]", round(roughness_mm, 6)),
            ("Kinematische Viskosität ν [m²/s]", nu),
            ("Reynolds-Zahl Re [-]", re),
            ("Relative Rauheit k/d [-]", epsilon_k),
    ...
    return overview, formulas, basis_text
...
with left:
    ...
    roughness_mm = st.number_input("Rohrrauhigkeit [mm]", value=0.0015, step=0.0001, format="%.4f")
...
with right:
    ...
    if run:
        try:
            overview_df, formulas_df, basis_text = calculate_lambda(fluid, float(temperature_c), float(pressure_bar), float(diameter_mm), float(velocity_ms), float(roughness_mm))
            st.dataframe(overview_df, use_container_width=True, hide_index=True)
            with st.expander("Formeln im Vergleich"):
                st.caption(f"Die ausgewählte Berechnungsgrundlage gehört zum Bereich: {basis_text}")
